...
export function TravelChat({ tripId: initialTripId, onPlanGenerated, onTripCreated }: TravelChatProps) {
  ...
  const createDraftTrip = useCallback(async () => {
    if (tripId || creatingRef.current) return null
  ...
  const transport = useMemo(
    () => new DefaultChatTransport({ api: "/api/travel/chat", body: { tripId } }),
    [tripId]
  )
  ...
  const { messages, sendMessage, status } = useChat({
    ...
    onToolCall({ toolCall }: any) {
      if (toolCall.toolName === "generate_trip_plan" && onPlanGenerated) {
        onPlanGenerated(toolCall.args as TripPlan)
      }
    },
  })
  ...
  const handleSend = async (content: string) => {
    // Create draft trip on first message if none exists
    let currentTripId = tripId
    if (!currentTripId) {
      currentTripId = await createDraftTrip()
